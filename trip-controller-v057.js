/* DRIVE v0.57 — authoritative trip controller
   This controller owns START/END DRIVE. It deliberately runs in capture phase so the legacy
   app.js trip handlers cannot race it. State is persisted in IndexedDB so reloads cannot
   strand an active drive.
*/
(function(){
  "use strict";
  ...
  const announce=message=>{const el=$("networkStatus")||$("recentActivity");if(el){el.dataset.message=message;el.setAttribute("aria-label",message);}};
  ...
  function ui(on){$("tripLiveCard")?.classList.toggle("hidden",!on);$("tripStartButton")?.classList.toggle("hidden",on);$("stopDriveButton")?.classList.toggle("hidden",!on);const b=$("startDriveButton");if(b)b.innerHTML=on?'<span class="action-icon" aria-hidden="true">●</span><span>DRIVING</span>':'<span class="action-icon" aria-hidden="true">●</span><span>START DRIVE</span>';state();}

  async function start(){
    if(active||finishInProgress)return;
    const saved=await read();
    if(saved?.status==="ACTIVE"){active={...saved,points:Array.isArray(saved.points)?saved.points:[],recovered:true};try{GPS.points=active.points.slice();GPS.distance=Number(active.distance||0);GPS.start();}catch(e){console.warn("DRIVE recovered GPS start failed",e);}ui(true);announce("Unfinished drive recovered.");return;}
    ...
    ui(true);announce("Drive recording. Data is saved locally.");emit();
  }
  ...
  async function finish(){
    ...
    try{
      ...
      active=null;
      ui(false);
      try{GPS.points=[];GPS.distance=0;}catch(e){}
      ...
      if(b){b.disabled=false;b.textContent="END DRIVE";}
  ...
  function bind(){if(bound)return;const startButton=$("startDriveButton"),stopButton=$("stopDriveButton");if(!startButton||!stopButton)return;bound=true;
    startButton.addEventListener("click",e=>{e.preventDefault();e.stopImmediatePropagation();Promise.resolve(start()).catch(e=>{console.error("DRIVE start failed",e);active=null;ui(false);announce(`Drive could not start: ${e?.message||"error"}`);});},true);
    stopButton.addEventListener("click",e=>{e.preventDefault();e.stopImmediatePropagation();Promise.resolve(finish()).catch(e=>console.error("DRIVE finish handler failed",e));},true);
  }
  async function boot(){bind();const saved=await read();if(saved?.status==="ACTIVE"){active={...saved,points:Array.isArray(saved.points)?saved.points:[],recovered:true};ui(true);try{GPS.points=active.points.slice();GPS.distance=Number(active.distance||0);GPS.start();}catch(e){console.warn("DRIVE recovery GPS start failed",e);}announce("Unfinished drive recovered.");}else ui(false);}
  document.addEventListener("gpsupdate",onGPS);
  document.addEventListener("DOMContentLoaded",()=>setTimeout(boot,0));
  window.DRIVE_TRIP_CONTROLLER={start,finish,recover:boot,isActive:()=>!!active,getActive:()=>active};
})();
