...
(function () {
  ...
  async function render(attempt, startedAt) {
    ...
    try {
      ...
      if (typeof openDatabase === "function") {
        try { await openDatabase(); } catch (error) { console.error("Analytics database readiness failed", error); }
      }
      ...
      if (kpis) kpis.innerHTML = cards.map(function (card) {
      ...
      var categories = {};
      data.month.expenses.forEach(function (r) { var category = r.category || "Other"; categories[category] = (categories[category] || 0) + Number(r.amount || 0); });
      data.month.maintenance.forEach(function (r) { categories.Maintenance = (categories.Maintenance || 0) + Number(r.cost || 0); });
      ...
      if (breakdown) breakdown.innerHTML = rows.length ? rows.map(function (row) {
      ...
      if (refreshQueued) {
        refreshQueued = false;
        if (view.classList.contains("active")) requestAnimationFrame(function () { render(0, Date.now()); });
      }
    }
  }
  ...
  /* v0.45.4: app.js owns Analytics navigation. Do not install a second click handler here. */
  window.DRIVE_ANALYTICS = { refresh: function () { if (byId("analyticsView")?.classList.contains("active")) render(0, Date.now()); }, open: openAnalytics };
})();
