...
async function setupFuelForm() {
    ...
    form.addEventListener("submit", async event => {
        ...
        if (odometer <= 0 || litres <= 0 || cost < 0) return;
        ...
        closeModal("fuelModal"); form.reset(); setDefaultDate(); await loadFuelHistory(); if (typeof initV06 === "function") await initV06();
    });
...
async function previewFuel() {
    ...
    if (!previous) return;
    const distance = odometer - Number(previous.odometer); if (distance <= 0) return;
    document.getElementById("fuelCalculation")?.classList.remove("hidden");
    const output = document.getElementById("calculatedEconomy"); if (output) output.textContent = (litres / distance * 100).toFixed(2);
}

async function loadFuelHistory() {
    const container = document.getElementById("fuelHistory"); if (!container) return;
    const active = typeof getActiveVehicle === "function" ? await getActiveVehicle() : null;
    const records = vehicleRecords(await getAllRecords("fuel"), active?.id).sort((a,b) => Number(b.odometer||0)-Number(a.odometer||0));
...
async function stopDrive() {
    ...
    GPS.stop(); clearInterval(tripTimer); const endedAt = Date.now();
    const active = typeof getActiveVehicle === "function" ? await getActiveVehicle() : null; const distance = Number(GPS.distance||0);
    ...
    tripActive=false; document.getElementById("tripLiveCard")?.classList.add("hidden"); const button=document.getElementById("startDriveButton"); if(button) button.textContent="● START DRIVE";
    alert(`Trip saved: ${distance.toFixed(2)} km`); GPS.points=[]; GPS.distance=0; if(typeof initV06 === "function") await initV06();
}
