...
(() => {
  ...
  async function analyze(){
    ...
    const signals=[];
    const watch=await window.DRIVE_WATCH?.collect?.();
  ...
  async function refresh(){const panel=target();if(!panel||!window.DRIVE_DATA?.intelligence)return;try{const result=await analyze();const watchSignals=result.watch?.watch||[];const combined=[...watchSignals.filter(x=>["attention","watch"].includes(x.severity)),...result.signals];const unique=combined.filter((x,i,a)=>a.findIndex(y=>y.title===x.title)===i).slice(0,2);panel.querySelector(".briefing-confidence")?.replaceChildren(document.createTextNode(result.confidence));const list=panel.querySelector(".briefing-list");if(!list)return;list.innerHTML=unique.map(x=>`<article class="briefing-item severity-${esc(x.severity)}"><div class="briefing-title"><strong>${esc(x.title)}</strong><span>${esc(x.severity).toUpperCase()}</span></div><p>${esc(x.text||x.detail)}</p><small>${esc(x.evidence||"")}</small></article>`).join("")||`<div class="briefing-empty"><strong>No immediate insight</strong><p>DRIVE is monitoring your vehicle history and will surface something when the recorded evidence supports it.</p></div>`}catch(error){console.error("DRIVE intelligence engine failed",error)}}
